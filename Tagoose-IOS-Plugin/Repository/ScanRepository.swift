...
class ScanRepository {
    ...
    var state: State {
        ...
        set {
            ...
            case .adjustingOrigin where stateValue == .scanning && qualityIsLow:
                ...
                let message = """
                This scan has not enough detail (it contains \(pointCloud.count) features - aim for at least \(ScanRepository.minFeatureCount)).
    ...
    func didOneFingerPan(_ gesture: UIPanGestureRecognizer) {
        ...
        if state == .defineBoundingBox || state == .scanning {
            ...
            case .changed:
                scannedObject.boundingBox?.updateSidePlaneDrag(screenPos: gesture.location(in: sceneView), sceneView: sceneView)
    ...
    func didTap(_ gesture: UITapGestureRecognizer, url:URL) {
        if state == .ready {
            state = .defineBoundingBox
        }
        ...
        if state == .defineBoundingBox || state == .scanning {
            if gesture.state == .ended {
                scannedObject.createOrMoveBoundingBox(screenPos: gesture.location(in: sceneView), url: url)
            }
        } else if state == .adjustingOrigin {
            if gesture.state == .ended {
    ...
    func updateOnEveryFrame(_ frame: ARFrame, center: CGPoint) {
