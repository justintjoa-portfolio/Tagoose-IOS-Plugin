...
class DetectedObject: SCNNode {
    ...
    func set3DModel(_ url: URL?, model:SCNNode, sceneView:ARSCNView) {
        if let url = url {
            customModel?.removeFromParentNode()
            customModel = nil
            originVis.removeFromParentNode()
            sceneView.prepare([model], completionHandler: { _ in
    ...
    init(referenceObject: ARReferenceObject, model:SCNNode, url: URL, sceneView: ARSCNView) {
        ...
        set3DModel(url, model: model, sceneView: sceneView)
    }
