...
class ScannedObject: SCNNode {
    ...
    func set3DModel(_ url: URL?, sceneView:ARSCNView) {
        self.origin?.set3DModel(url, extentForScaling: boundingBox?.extent, sceneView: sceneView)
    }
    
    func createOrMoveBoundingBox(screenPos: CGPoint, url:URL) {
        if let boundingBox = self.boundingBox {
            ...
            createBoundingBox(screenPos: screenPos, url: url)
        }
    }
    
    func createBoundingBoxFromGhost(url:URL) {
        if let boundingBox = self.ghostBoundingBox {
            ...
            let origin = ObjectOrigin(extent: boundingBox.extent, sceneView, url:url)
    ...
    func fitOverPointCloud(_ pointCloud: ARPointCloud, center:CGPoint) {
    ...
    func createBoundingBox(screenPos: CGPoint, url:URL) {
        // Perform a hit test against the feature point cloud.
        guard let result = sceneView.smartHitTest(screenPos) else {
            print("Warning: Failed to find a position for the bounding box.")
            return
    ...
    private func updateOrCreateGhostBoundingBox(center:CGPoint) {
        // Perform a hit test against the feature point cloud.
    ...
    func updateOnEveryFrame(center:CGPoint) {
        if let boundingBox = boundingBox {
            ...
            updateOrCreateGhostBoundingBox(center:center)
        }
    }
